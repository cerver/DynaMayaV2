...
namespace DynaMaya.Util
{
    ...
    public class UINodeUtility
    {
        ...
        public static List<Curve> GetMayaCurve()
        {
            ...
            var listAsDags = selectionList.DagPaths(MFn.Type.kNurbsCurve);
            List<Curve> outCurves = new List<Curve>((int)selectionList.length);
            outCurves.AddRange(listAsDags.Select(itm => DMInterop.MTDCurveFromDag(itm, 0)));

            return outCurves;
        }

        public static CurveListNode BuildCurveListNode(List<Curve> value)
        {
            return new CurveListNode(value);
        }

    }

    public class CurveListNode : AssociativeNode
    {
        public List<Curve> Value { get; set; }

        public CurveListNode(List<Curve> value)
        {
            this.IsLiteral = true;
            this.Value = value;
        }

        public CurveListNode(CurveListNode rhs)
          : base((AssociativeNode)rhs)
        {
            this.IsLiteral = true;
        ...
        {
            CurveListNode doubleNode = other as CurveListNode;
            if (doubleNode == null)
                return false;
            return this.Value.Equals(doubleNode.Value);
        }
